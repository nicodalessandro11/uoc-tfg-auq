...
// Import the API service functions instead of mock data
import { getDistricts, getGeoJSON } from "@/lib/api-service"
// Add the import for the Supabase functions at the top of the file
import { getDistrictPolygons, getNeighborhoodPolygons } from "@/lib/supabase-client"
...
type MapContextType = {
  ...
  resetFilters: () => void
  visiblePointTypes: Record<PointFeatureType, boolean>
  setVisiblePointTypes: (types: Record<PointFeatureType, boolean>) => void
  currentGeoJSON: any
...
let globalVisiblePointTypes: Record<PointFeatureType, boolean> | null = null
let globalFilters: Filters | null = null
let globalHasSelectedGranularity = false
let globalMapType: MapType = "osm"

// Update the defaultVisiblePointTypes to include all feature types
const defaultVisiblePointTypes: Record<PointFeatureType, boolean> = {
  "1": true,
  "2": true,
  "3": true,
  "4": true,
...
export function MapProvider({ children }: { children: ReactNode }) {
  ...
  const [visiblePointTypes, _setVisiblePointTypes] = useState<Record<PointFeatureType, boolean>>(
  ...
  const loadAvailableAreas = useCallback(async (cityId: number, granularityLevel: string) => {
    ...
    try {
      ...
      if (granularityLevel === "district") {
        ...
        areas = districtsData.map((district) => ({
          id: district.id,
          name: district.name,
      ...
      } else if (granularityLevel === "neighborhood") {
        // For neighborhoods, use the direct Supabase data
        try {
          const geoJsonData = await getNeighborhoodPolygons(cityId)
  ...
  const loadGeoJSON = useCallback(
    async (cityId: number, granularityLevel: string) => {
      ...
      try {
        // Load polygon data directly from Supabase for districts and neighborhoods
        let data
        if (granularityLevel === "district") {
          data = await getDistrictPolygons(cityId)
        } else if (granularityLevel === "neighborhood") {
          data = await getNeighborhoodPolygons(cityId)
        } else {
          // Fallback to API for other data types
          data = await getGeoJSON(cityId, granularityLevel)
        }
  ...
  const setVisiblePointTypes = useCallback(
    (types: Record<PointFeatureType, boolean>) => {
      // Only update if values actually changed
